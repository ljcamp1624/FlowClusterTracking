...
%
function [xyt, xyt2, tracks] = ClusterTrack(peakIm, clusterIm, mask, peakThresh, peakSize, maxDisp)
...
for i = 1:size(clusterIm, 3)
    pks = pkfnd(z(:, :, i), peakThresh, 2*peakSize);
...
xyt2 = [];
for i = 1:size(clusterIm, 3)
    pks2 = cntrd(clusterIm(:, :, i), xyt(xyt(:, 3) == i, :), 2*peakSize + 1);
    if ~isempty(pks2)
        pks2 = pks2(:, [1, 2]);
    end
